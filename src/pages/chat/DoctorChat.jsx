...
const DoctorChat = () => {
  ...
  try {
    ...
    // 🔥 Evita crash no erro 500
    if (!response.ok) {
      console.error("Backend error:", response.status);
      setContacts([]);      // limpa contatos
      return;               // sai sem quebrar o app
    }

    const data = await response.json();

    // 🔥 Evita erro caso o backend retorne algo inesperado
    if (!data || !data.Médicos || !Array.isArray(data.Médicos)) {
      setContacts([]);
      return;
    }
    ...
    const mappedContacts = data.Médicos.map(medico => ({
      id: medico.id_user || medico.id || Math.random().toString(),
      nome: medico.nome_user || medico.name || 'Sem nome',
      tipo: 'Responsável'
    }));
    ...
    setContacts([]);     // mantém tudo funcionando mesmo quando dá erro
  } finally {
